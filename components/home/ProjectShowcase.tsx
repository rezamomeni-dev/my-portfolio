      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="grid gap-16 md:gap-24">
          {projectsData.map((project, index) => (
            ...
            >
              <div className={`relative aspect-[4/3] rounded-3xl overflow-hidden bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-2xl group ${index % 2 === 1 ? 'md:order-last' : ''}`}>
              ...
              <div className="flex flex-col">
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.technologies.slice(0, 4).map((tech, i) => (
                    <span key={i} className="px-4 py-1.5 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-sm font-medium">
                      {tech}
                    </span>
                  ))}
                </div>
        ...
        <div className="mt-16 text-center">
          ...
          >
            See all projects in resume <ArrowUpRight className="w-5 h-5" />
          </Link>
        </div>
      </div>
    </section>
