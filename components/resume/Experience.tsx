  ...
  return (
    <section id="experience" className="py-16 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <h2 className="text-3xl font-bold text-zinc-900 dark:text-white mb-8">Experience</h2>
            ...
            {items.map((item, index) => (
              <div key={index} className="flex flex-col md:flex-row gap-8 md:gap-24">
                <div className="md:w-48 shrink-0">
                  <span className="text-zinc-500 dark:text-zinc-500 text-sm font-medium uppercase tracking-wider">{item.period}</span>
                ...
                <div className="flex-1 group">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-xl font-bold text-zinc-900 dark:text-white group-hover:text-primary transition-colors flex items-center gap-2">
                      {item.role} <ArrowUpRight className="w-4 h-4 text-primary opacity-0 group-hover:opacity-100 transition-opacity" />
                    </h3>
                  </div>
                  <p className="text-zinc-600 dark:text-zinc-400 font-medium mb-4">{item.company}</p>

                  <ul className="space-y-3 mb-6">
                    {item.description.map((desc, i) => (
                      <li key={i} className="text-zinc-600 dark:text-zinc-500 text-sm leading-relaxed flex gap-2">
