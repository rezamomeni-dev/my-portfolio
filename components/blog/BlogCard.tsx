...
export const BlogCard = ({ post, index }: BlogCardProps) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className="group relative flex flex-col h-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all"
    >
      <div className="p-6 flex flex-col h-full">
        ...
        <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-2 line-clamp-2 group-hover:text-primary transition-colors">
          <a href={post.link} target="_blank" rel="noopener noreferrer" className="after:absolute after:inset-0">
            {post.title}
          </a>
        </h3>
        ...
        <div className="flex items-center justify-between mt-auto pt-4 border-t border-zinc-100 dark:border-zinc-800">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-zinc-500 dark:text-zinc-400">{post.category}</span>
          </div>
